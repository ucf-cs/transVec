...
{
	// All other cases.
	size_t first = pos / Page::SEG_SIZE;
	size_t second = pos % Page::SEG_SIZE;
...
void RWSet::setToPages(Desc *descriptor)
{
	...
	std::map<size_t, Page *, std::less<size_t>, MyPageAllocator> *pages = new std::map<size_t, Page *, std::less<size_t>, MyPageAllocator>();
	...
	for (auto i = operations.begin(); i != operations.end(); ++i)
	{
		...
		Page *page = Allocator<Page>::alloc();
	...
	std::map<size_t, Page *, std::less<size_t>, MyPageAllocator> *nullVal = NULL;
	descriptor->pages.compare_exchange_strong(nullVal, pages);

	return;
}

void RWSet::setOperationVals(Desc *descriptor, std::map<size_t, Page *, std::less<size_t>, MyPageAllocator> pages)
{
